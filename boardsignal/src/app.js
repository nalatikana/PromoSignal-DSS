/* ============================================================================
   ...
   โครงสร้าง
     ...
     2. สถานะผู้ใช้ · เครดิต · ผนังจ่ายเงิน
     3. หน้าจอ 8 หน้า
...
/** IPI = ครึ่งหนึ่งของเปอร์เซ็นไทล์ CEO + ครึ่งหนึ่งของเปอร์เซ็นไทล์บอร์ด */
...
/* ---------------------------------------------------------------- 2 · สถานะ */
const PRICE = { peer: 1, studio: 1, ceotext: 1, report: 1, sector: 5 };
const PACKS = [
  { n: 5, baht: 490, tag: "" },
  { n: 15, baht: 1290, tag: "คุ้มที่สุด", save: 12 },
  { n: 40, baht: 2990, tag: "สำหรับทีม", save: 24 },
];
const S = {
  user: null, credits: 3, freeCo: null,      // บริษัทที่ดูฟรีได้ตลอด (บริษัทแรก)
  unlocked: {},                              // { "peer:PTG": true, ... }
  sectors: {},                               // อุตสาหกรรมที่ซื้อแล้ว
  log: [],                                   // ประวัติการใช้เครดิต
  ind: "all", t: "PTG",
  peers: [], wi: null, ceo: null,
  view: "fit",
};
const LS = "boardsignal_demo_v1";
function save() { try { localStorage.setItem(LS, JSON.stringify({ u: S.user, c: S.credits, f: S.freeCo, k: S.unlocked, s: S.sectors, l: S.log.slice(0, 40), b: document.body.dataset.brand, m: document.body.dataset.mode })); } catch (e) { } }
function load() {
  try {
    ...
    if (o.b) document.body.dataset.brand = o.b;
...
/** ใช้เครดิต — คืน true ถ้าปลดล็อกสำเร็จ */
function spend(what, t, label) {
  t = t || S.t;
  const cost = PRICE[what] || 1;
  if (S.credits < cost) { go("billing"); toast("เครดิตไม่พอ — เหลือ " + S.credits + " เครดิต ต้องใช้ " + cost); return false; }
  S.credits -= cost;
  ...
  save(); syncChrome(); toast("ปลดล็อกแล้ว · ใช้ " + cost + " เครดิต · เหลือ " + S.credits);
  return true;
}
let toastT;
function toast(msg) {
...
const NAV = [
  ...
  { id: "billing", g: 2, ico: "◈", lab: "เครดิตและการชำระเงิน" },
...
/* ---------------------------------------------------------------- แถบบน */
function syncChrome() {
  const bn = { A: "Board Signal", B: "BoardSignal", C: "BOARDSIGNAL" }[document.body.dataset.brand] || "Board Signal";
  ["#sBrand", "#gBrand", "#fBrand"].forEach(s => { const e = $(s); if (e) e.textContent = bn; });
  $("#chipCredit").innerHTML = `<span aria-hidden="true">◈</span> เครดิต <b>${S.credits}</b>`;
  const c = cur(), z = ZONES[zoneOf(c.promo, c.amp)];
  $("#chipZone").innerHTML = `<span style="width:8px;height:8px;border-radius:99px;background:${z.c};display:inline-block"></span> ${z.l} · IPI <b>${c.ipi}</b>`;
  $("#sUser").textContent = S.user || "";
  $("#sPlan").textContent = `เครดิตคงเหลือ ${S.credits} · ${Object.keys(S.sectors).length} อุตสาหกรรมที่ซื้อแล้ว`;
  syncNavLocks();
}
  ...
  return `<div class="lockwrap">
    ...
    <div class="paywall" style="align-items:flex-start;padding-top:64px"><div class="paycard">
      ...
      <div style="display:flex;gap:9px;justify-content:center;flex-wrap:wrap">
        <button class="btn p" data-unlock="${what}">ปลดล็อก · ใช้ ${cost} เครดิต</button>
        <button class="btn s" data-goto="billing">ดูแพ็กเกจเครดิต</button>
      </div>
      <div style="font-size:11px;color:var(--ink-3);margin-top:11px">
        คุณมี <b style="color:var(--ink)">${S.credits}</b> เครดิต${tag ? " · " + tag : ""}</div>
    </div></div></div>`;
}
...
function viewFit() {
    ...
    <div style="display:grid;gap:14px;align-content:start">
      <div class="card">
        ...
        <div class="note brand" style="margin-top:13px">
          <b>IPI = ครึ่ง CEO + ครึ่งบอร์ด</b> — ${c.dp} และ ${ampPct(c.amp)} เฉลี่ยกันได้ ${c.ipi}
  ...
  if (isUnlocked("report")) {
    const badge = S.freeCo === c.t ? `<span class="chip free">◇ รายงานฟรีของคุณ</span>` : `<span class="chip">เปิดด้วยเครดิตแล้ว</span>`;
  ...
  return head + (firstFree
    ...
    : paywall(body, "report", "รายงานวินิจฉัยฉบับเต็ม",
      `คุณใช้สิทธิ์รายงานฟรีไปกับ <b>${esc(S.freeCo)}</b> แล้ว — เปิดรายงานของ <b>${esc(c.t)}</b> ด้วยเครดิต 1 หน่วย
       จะได้ดัชนี IPI ค่าองค์ประกอบบอร์ดเทียบอุตสาหกรรม และข้อเสนอแนะรายบริษัท`, PRICE.report,
      "สิทธิ์ฟรีใช้ไปแล้ว"));
}

/** ข้อเสนอแนะเชิงตัวเลข — เรียงตามผลกระทบต่อ IPI */
function nextSteps(c) {
  ...
  if (!c.pol) mk("มีกรรมการที่มีเครือข่ายเชิงนโยบาย", "สัมพันธ์กับการเข้าถึงทรัพยากรและพันธมิตรภาครัฐ (β = " + fmt(BETA.pXpol, 2) + ")",
    c.fam, 1, c.fem, c.ten);
  if (c.dp < 85) mk("CEO ยกระดับภาษาเชิงรุก +15 เปอร์เซ็นไทล์", "เป็นคานงัดอีกครึ่งหนึ่งของ IPI ที่ไม่ต้องแก้โครงสร้างบอร์ด",
  ...
  return out.slice(0, 5).map(o => `<div style="display:flex;gap:12px;align-items:flex-start;padding:9px 0;border-bottom:1px solid var(--line)">
    ...
    <div class="pill ${o.d > 0 ? "ok" : "mute"}" style="white-space:nowrap;font-size:11px">
      ${o.d > 0 ? "▲ +" + o.d : o.d < 0 ? "▼ " + o.d : "±0"} → IPI ${o.v}</div></div>`).join("");
}
...
function viewMatrix() {
  ...
  <div class="card">
    ...
    <svg viewBox="0 0 ${W} ${H}" width="100%" id="mxSvg" role="img" aria-label="เมทริกซ์ 2 คูณ 2">
      ...
      <text x="${(X(c.promo) + 14).toFixed(1)}" y="${(Y(c.amp) + 4).toFixed(1)}" font-size="12" font-weight="500" fill="var(--ink)">${esc(c.t)} · IPI ${c.ipi}</text>
  ...
  <div class="grid g-side-r" style="margin-top:14px">
    <div class="card">
      ...
      : `<div class="tw"><table><thead><tr><th>แกน</th><th class="n">ตอนนี้</th><th class="n">เป้าหมาย</th><th class="n">ส่วนต่าง</th><th>แปลว่า</th></tr></thead><tbody>
          ...
          <tr class="hl"><td><b>IPI</b></td><td class="n"><b>${c.ipi}</b></td><td class="n"><b>${tgtIPI}</b></td>
...
const ROWS_CMP = [
  { k: "ipi", l: "IPI (ดัชนีรวม)", f: c => c.ipi, d: 0, hi: 1, big: 1 },
...
function viewPeer() {
  ...
  <div class="grid g2">
    ...
    <div class="card">
      <h3>ตำแหน่ง IPI ในการกระจายตัว</h3>
...
function peerVerdict(c, peers) {
  ...
  return `<div class="grid g3 tight" style="gap:10px;margin-bottom:12px">
    <div class="stat"><div class="k">คู่แข่งที่นำอยู่</div><div class="v">${better.length}<span style="font-size:15px;color:var(--ink-3)"> / ${peers.length}</span></div>
      <div class="u">${better.length ? better.map(p => p.t).join(" · ") + " มี IPI สูงกว่า" : "ไม่มีคู่แข่งในกลุ่มนี้ที่คะแนนสูงกว่า"}</div></div>
    <div class="stat"><div class="k">ห่างจากผู้นำกลุ่มเทียบ</div><div class="v" style="color:${gap > 0 ? "var(--crit)" : "var(--ok)"}">${gap > 0 ? "−" + gap : "+" + Math.abs(gap)}</div>
      <div class="u">${esc(lead.t)} อยู่ที่ IPI ${lead.ipi}</div></div>
  ...
  <div class="note ${gap > 0 ? "warn" : "ok"}">
    ${gap > 0
      ? `<b>ถ้าจะไล่ให้ทัน ${esc(lead.t)} ต้องเพิ่ม IPI อีก ${gap} คะแนน</b> — ช่องว่างส่วนใหญ่มาจาก${which}
...
function viewStudio() {
  ...
  <div class="card" style="margin-bottom:14px">
    ...
    <div class="grid g4 tight" style="gap:10px;margin:10px 0 14px">
      <div class="stat"><div class="k">IPI ปัจจุบัน</div><div class="v num">${c.ipi}</div><div class="u">${ZONES[z0].l}</div></div>
      <div class="stat hero"><div class="k">IPI จำลอง</div><div class="v num">${ipi1}</div><div class="u">${ZONES[zone1].l}</div></div>
    ...
    ${boxplot(IPI_ALL, ipi1, "IPI · 196 บริษัท")}
  ...
  <div class="grid g-side-r">
    <div class="card">
      ...
      <p class="desc">คำนวณจากสถานะจำลองปัจจุบัน · เรียงตามผลกระทบต่อ IPI</p>
      ${wiScenarios(c, w)}
    </div>
    <div class="card">
  ...
  const head = pageHead("จำลองสถานการณ์", "What-if Studio",
    `ปรับได้ทั้งสองด้านพร้อมกัน — ภาษาของ CEO จากเอกสารจริง และองค์ประกอบคณะกรรมการ — แล้วดูว่า IPI ขยับไปเท่าไหร่และย้ายโซนหรือไม่`);
...
function wiScenarios(c, w) {
  ...
  return out.slice(0, 5).map(o => `<div style="display:flex;gap:12px;align-items:center;padding:9px 0;border-bottom:1px solid var(--line)">
    ...
    <div class="num" style="font-size:13px;color:var(--ink-2)">IPI ${o.v}</div></div>`).join("");
}
function persona(w, ipi) {
  ...
  return `<div style="font-size:12.8px;line-height:1.85">
    ณ สถานะจำลองนี้ (IPI ${ipi} · CEO Drive ${w.dp} · กรรมการหญิง ${pct1(w.fem)} · ครอบครัว ${pct1(w.fam)} · อายุงาน ${fmt(w.ten, 1)} ปี)
...
function viewSector() {
  ...
  <div class="grid g4 tight" style="gap:10px;margin-bottom:14px">
    ...
    <div class="stat"><div class="k">IPI มัธยฐานของกลุ่ม</div><div class="v num">${fmt(st.med)}</div>
  ...
  <div class="card" style="margin-bottom:14px">
    ...
    <p class="desc">คลิกแถวใดก็ได้เพื่อเปิดรายงานของบริษัทนั้น · แถบใต้ IPI คือตำแหน่งเทียบทั้งตลาด</p>
    <div class="tw" style="max-height:520px;overflow:auto"><table>
      <thead style="position:sticky;top:0;background:var(--panel);z-index:2"><tr>
        <th class="n">#</th><th>บริษัท</th><th>โซน</th><th class="n">IPI</th>
  ...
  <div class="grid g2">
    <div class="card"><h3>การกระจายตัวของ IPI ในกลุ่ม</h3>
...
/* ============================================================================
   หน้า 6 · เครดิตและการชำระเงิน
   ============================================================================ */
function viewBilling() {
  const used = S.log.reduce((a, b) => a + b.cost, 0);
  return pageHead("บัญชีของคุณ", "เครดิตและการชำระเงิน",
    "ระบบคิดค่าบริการแบบตัดเครดิตรายครั้ง ไม่มีค่าสมาชิกรายเดือน — ผู้ใช้จ่ายเฉพาะรายงานที่เปิดจริง")
    + `<div class="grid g4 tight" style="gap:10px;margin-bottom:16px">
      <div class="stat hero"><div class="k">เครดิตคงเหลือ</div><div class="v num">${S.credits}</div>
    ...
    <div class="card" style="margin-bottom:14px">
      <h3>แพ็กเกจเครดิต</h3>
      <p class="desc">1 เครดิต = เปิดรายงาน 1 ครั้ง · เครดิตไม่หมดอายุ · ต้นแบบนี้จำลองการชำระเงิน ไม่มีการตัดบัตรจริง</p>
      <div class="grid g3">${PACKS.map((p, i) => `
        <div style="border:1px solid ${i === 1 ? "var(--brand)" : "var(--line)"};border-radius:var(--radius);padding:17px;position:relative;background:${i === 1 ? "color-mix(in srgb,var(--brand) 5%,var(--panel))" : "var(--panel)"}">
          ...
          <div style="font-size:29px;font-weight:500;letter-spacing:-.03em">${p.n}<span style="font-size:14px;color:var(--ink-3);font-weight:300"> เครดิต</span></div>
      ...
      <div class="note brand" style="margin-top:14px">
        <b>ทำไมคิดเป็นเครดิต ไม่ใช่รายเดือน</b> — ผู้ใช้กลุ่มเป้าหมาย (ที่ปรึกษา HRD · นักวิเคราะห์ · ฝ่ายเลขานุการบริษัท)
        ใช้งานเป็นครั้ง ๆ ตามดีลหรือตามรอบประชุม การคิดรายเดือนจึงเป็นกำแพงเข้าใช้งาน
        ส่วนเครดิตทำให้ทดลองก่อนแล้วค่อยจ่ายตามที่ใช้จริงได้</div>
    </div>

    <div class="grid g2">
      <div class="card"><h3>อัตราค่าบริการ</h3>
        <div class="tw"><table><thead><tr><th>รายการ</th><th class="n">เครดิต</th><th>หมายเหตุ</th></tr></thead><tbody>
      ...
      <div class="card"><h3>ประวัติการใช้เครดิต</h3>
        <p class="desc">ระบบจริงจะออกใบเสร็จอิเล็กทรอนิกส์ทุกรายการ</p>
        ${S.log.length ? `<div class="tw" style="max-height:260px;overflow:auto"><table><thead><tr><th>รายการ</th><th>อ้างอิง</th><th class="n">เครดิต</th><th>เวลา</th></tr></thead><tbody>
      ...
      : `<div style="text-align:center;padding:34px 10px;color:var(--ink-3);font-size:12.5px">ยังไม่มีการใช้เครดิต</div>`}
        <button class="btn g" id="btnResetDemo" style="margin-top:12px;width:100%;justify-content:center">รีเซ็ตข้อมูลสาธิต (คืนเครดิต 3 และล้างสิทธิ์)</button>
      </div>
    </div>`;
}
  ...
  return pageHead("ระบบและหลักฐาน", "สถาปัตยกรรมข้อมูลและแผนพัฒนา",
    ...
    + `<div class="card" style="margin-bottom:14px">
      ...
      <svg viewBox="0 0 900 340" width="100%" role="img" aria-label="แผนภาพสถาปัตยกรรม">
        ...
        ${box(298, 96, 210, 108, "ชั้นประมวลผล", ["คำนวณ CEO Drive และ Board Amp", "จัดเปอร์เซ็นไทล์เทียบตลาด", "รวมเป็นดัชนี IPI 0–100"], "color-mix(in srgb,var(--brand) 9%,var(--panel))", "var(--brand)")}
        ...
        ${box(590, 218, 300, 84, "ภาพรวมอุตสาหกรรม", ["ตารางจัดอันดับ · ส่งออก CSV", "คิดค่าบริการรายครั้งหรือตัดเครดิต"], "color-mix(in srgb,var(--accent) 12%,var(--panel))", "var(--accent)")}
      </svg>
    </div>
    ...
    <div class="grid g3" style="margin-bottom:14px">
      ...
      "ล็อกอิน · เครดิต · ผนังจ่ายเงิน · ประวัติการใช้",
...
function viewValid() {
  ...
  const dec = V.decileB;
  const mxd = Math.max(...dec);
  return pageHead("ระบบและหลักฐาน", "หลักฐานว่าคะแนนใช้ได้จริง",
    "หน้านี้เปิดให้ดูฟรีเสมอ — ผู้ซื้อควรตรวจได้ก่อนจ่ายว่าคะแนนมีหลักฐานรองรับแค่ไหน และมีข้อจำกัดตรงไหน")
    + `<div class="grid g4 tight" style="gap:10px;margin-bottom:14px">
      <div class="stat hero"><div class="k">กลุ่มคะแนนสูงสุด เทียบ ต่ำสุด</div><div class="v num">${fmt(V.lift, 2)}×</div>
        <div class="u">บริษัทกลุ่มบนมีกิจกรรมนวัตกรรมเปิดมากกว่ากลุ่มล่างเท่านี้</div></div>
      <div class="stat"><div class="k">AUC</div><div class="v num">${fmt(V.auc, 2)}</div>
        <div class="u">แยกบริษัทนวัตกรรมสูงได้ · สุ่ม = 0.50</div></div>
      <div class="stat"><div class="k">สหสัมพันธ์อันดับ</div><div class="v num">${fmt(V.spearman, 3)}</div>
        <div class="u">ทั้งแผงข้อมูล 928 บริษัท-ปี</div></div>
      <div class="stat"><div class="k">นอกกลุ่มตัวอย่าง (LOYO)</div><div class="v num">+${fmt(V.loyo, 2)}</div>
        <div class="u">เฉลี่ยจากการกันข้อมูลออกทีละปี 5 รอบ</div></div>
    </div>
    ...
    <div class="card" style="margin-bottom:14px">
      <h3>ความแม่นรายปี — ปีไหนสัญญาณชัด ปีไหนหาย</h3>
      <p class="desc">เกณฑ์ผ่าน: สหสัมพันธ์อันดับ ≥ ${th.sp[0]} · AUC ≥ ${th.auc[0]} · อัตราส่วนกลุ่มบนต่อล่าง ≥ ${th.lift[0]}</p>
      <div class="tw"><table><thead><tr><th>ปี</th><th class="n">n</th>
        <th class="n">สหสัมพันธ์อันดับ</th><th class="n">AUC</th><th class="n">กลุ่มบน/ล่าง</th><th>สรุป</th></tr></thead><tbody>
        ${yrs.map(([y, v]) => {
          const g = [grade(v.sp, th.sp), grade(v.auc, th.auc), grade(v.lift, th.lift)];
          const worst = g.some(x => x[1] === "crit") ? ["ไม่ผ่าน", "crit"] : g.some(x => x[1] === "warn") ? ["ควรระวัง", "warn"] : ["ผ่านทั้งสามเกณฑ์", "ok"];
          return `<tr><td><b>${+y + 543}</b> <span style="color:var(--ink-3)">(${y})</span></td><td class="n">${v.n}</td>
            ${[["sp", v.sp, 3], ["auc", v.auc, 3], ["lift", v.lift, 2]].map(([k, val, d]) => {
            const gg = grade(val, th[k]);
            return `<td class="n"><span style="color:${gg[1] === "ok" ? "var(--ok)" : gg[1] === "warn" ? "var(--warn)" : "var(--crit)"}">${fmt(val, d)}</span></td>`;
          }).join("")}
            <td><span class="pill ${worst[1]}">${worst[1] === "ok" ? "✓" : worst[1] === "warn" ? "⚠" : "✕"} ${worst[0]}</span></td></tr>`;
        }).join("")}
        <tr class="hl"><td><b>รวมทุกปี</b></td><td class="n">${VY.all.n}</td>
          ...
          <td class="n"><b>${fmt(VY.all.lift, 2)}</b></td><td><span class="pill ok">✓ ผ่านทั้งสามเกณฑ์</span></td></tr>
      </tbody></table></div>
      <div class="note warn" style="margin-top:12px">
        <b>ปี 2565 สัญญาณหายไปเกือบหมด</b> (สหสัมพันธ์ −0.02 · AUC 0.49 ≈ เดาสุ่ม) —
        ต้องพูดตรง ๆ ในการนำเสนอ ไม่ควรรายงานแค่ค่าเฉลี่ยรวม ปีที่เหลืออีก 4 ปีสัญญาณชัดและไปในทิศทางเดียวกัน
        ข้อสันนิษฐานคือผลกระทบจากสถานการณ์เศรษฐกิจในปีนั้น ซึ่งควรตรวจเพิ่มก่อนสรุป</div>
    </div>
    ...
    <div class="card">
      ...
      <div class="grid g2" style="gap:12px;margin-top:10px">
        ...
        <div class="note"><b>2 · ขนาดของสัญญาณปานกลาง</b><br>
          AUC 0.61 แปลว่าดีกว่าเดาสุ่มอย่างมีนัยสำคัญ แต่ยังห่างจากเครื่องมือที่ใช้ตัดสินใจเดี่ยว ๆ ได้
...
function wireMatrix(h) {
  ...
  h.querySelectorAll(".pt").forEach(p => {
    tipOn(p, el => `<div class="tk">${el.dataset.z}</div><b>${el.dataset.t}</b> · ${el.dataset.n}<br>IPI ${el.dataset.i} — คลิกเพื่อเปิดรายงาน`);
...
function wirePeer(h) {
  ...
  if (box) {
    box.innerHTML = [0, 1, 2].map(i => `<select class="sel" data-slot="${i}" style="font-size:12px;padding:5px 9px">
      ...
      ${pool.map(x => `<option value="${x.t}"${S.peers[i] === x.t ? " selected" : ""}>${esc(x.t)} · IPI ${x.ipi}</option>`).join("")}</select>`).join("");
...
function wireSector(h) {
  ...
  if (cs) cs.onclick = () => {
    ...
    const head = ["อันดับ", "ตัวย่อ", "ชื่อบริษัท", "อุตสาหกรรม", "โซน", "IPI", "CEO_Drive", "Board_Amp_pct", "Board_Amp_raw",
...
function wireBilling(h) {
  h.querySelectorAll("[data-buy]").forEach(b => b.onclick = () => {
    ...
    if (!confirm(`ต้นแบบ — จำลองการชำระเงิน\n\nแพ็กเกจ ${n} เครดิต · ฿${fmt(p.baht)}\nระบบจริงจะพาไปหน้าชำระเงิน (พร้อมเพย์ / บัตรเครดิต)\n\nกดตกลงเพื่อจำลองว่าชำระสำเร็จ`)) return;
    S.credits += n; S.log.unshift({ what: "topup", t: "-", cost: -n, label: `เติม ${n} เครดิต · ฿${fmt(p.baht)}`, at: new Date().toLocaleString("th-TH", { dateStyle: "short", timeStyle: "short" }) });
    save(); toast(`เติม ${n} เครดิตแล้ว · คงเหลือ ${S.credits}`); render();
  });
...
/* ---------------------------------------------------------------- แบรนด์ */
const BRANDS = [
  { k: "A", n: "Ink & Signal", d: "น้ำเงินเข้ม + ทองอำพัน — โทนสถาบันการเงินและที่ปรึกษา น่าเชื่อถือ เหมาะกับการเสนอกรรมการบริษัท" },
  { k: "B", n: "Teal Board", d: "เขียวน้ำทะเล + ส้มอิฐ — ใกล้เคียงตัวอย่างเดิม อ่านสบายตา เหมาะกับงานวิชาการ" },
  { k: "C", n: "Graphite & Lime", d: "ดำกราไฟต์ + เขียวมะนาว — โทนซอฟต์แวร์สมัยใหม่ คมและจำง่าย เหมาะกับการวางตำแหน่งเป็นผลิตภัณฑ์" },
];
...
function boot() {
  buildNav();
  $("#selBrand").innerHTML = BRANDS.map(b => `<option value="${b.k}">ชุด ${b.k} · ${b.n}</option>`).join("");
  $("#gateBrands").innerHTML = BRANDS.map(b =>
    `<button class="btn s" data-brand="${b.k}" style="font-size:11.5px;padding:6px 11px" title="${esc(b.d)}">ชุด ${b.k} · ${b.n}</button>`).join("");
  $$("[data-brand]").forEach(b => { if (b.dataset.brand && b.tagName === "BUTTON") b.onclick = () => { setBrand(b.dataset.brand); $("#selBrand").value = b.dataset.brand; }; });
  $("#selBrand").onchange = e => setBrand(e.target.value);
  ...
  const u = load();
  $("#selBrand").value = document.body.dataset.brand;
  syncChrome();
