...
const MODES = {
  ...
  inv: { lab: "คัดหุ้นเข้าพอร์ต", sub: "ดูคะแนนและคลาส → ตรวจย้อนหลังเทียบ SET → shortlist ตรวจต่อ", tabs: ["iv", "bt", "live"] },
};
...
function renderMarketBacktestBlock() {
  ...
  if (!mv) return `<div class="note warn" style="margin-bottom:16px"><b>ยังไม่มี market backtest</b><br>
    ต้องประมวลผลข้อมูลย้อนหลังของหุ้นและ SET Index ก่อน จึงจะแสดงผลเทียบพอร์ตจำลองได้</div>`;
  const m = mv.backtest.metrics;
  return `
    <h3 style="margin:0 0 8px">ผลทดสอบย้อนหลัง: หุ้นคะแนนสูงสุด 10 ตัว เทียบ SET Index</h3>
    <div class="tiles" style="margin-bottom:16px">
      <div class="tile hero"><div class="lab">ผลตอบแทนเหนือ SET รวม</div><div class="val">${moneyPct(m.alpha_pct)}</div>
        <div class="sub">${esc(mv.backtest.portfolio)} · ${esc(mv.backtest.period)}</div></div>
      <div class="tile"><div class="lab">ผลตอบแทนกลุ่มที่ระบบคัดเลือก</div><div class="val">${moneyPct(m.portfolio_total_return_pct)}</div>
        <div class="sub">SET Index ${moneyPct(m.set_total_return_pct)}</div></div>
      <div class="tile"><div class="lab">ชนะ SET รายไตรมาส</div><div class="val">${fmt(m.win_rate_pct, 0)}%</div>
    ...
    <div class="softbox" style="margin-bottom:16px">
      <h3>กราฟมูลค่าพอร์ตจำลอง</h3>
      <p class="desc">ฐาน 100 · ปรับพอร์ตทุกไตรมาส · ถือหุ้นที่ได้คะแนนสูงสุด 10 ตัว แบบน้ำหนักเท่ากัน</p>
      ${renderEquityChart(mv)}
    </div>
    <table><thead><tr><th>ช่วงเวลา</th><th class="n">หุ้นที่ระบบคัดเลือก</th><th class="n">SET Index</th><th class="n">ส่วนต่าง</th><th>ผล</th></tr></thead><tbody>
    ...
    <div class="note ${mv.status === "demo_generated" ? "warn" : "ok"}" style="margin:12px 0 20px">
      <b>${mv.status === "demo_generated" ? "ข้อมูลสาธิตสำหรับนำเสนอ" : "ข้อมูลจากรอบประมวลผลจริง"}</b><br>
      ${esc(mv.data_note)}<br>
      ขั้นตอนคำนวณ: ${mv.pipeline.map(esc).join(" → ")}
    </div>`;
}
...
function renderLatestTop5() {
  ...
  if (!mv?.latest?.rows?.length) {
    host.innerHTML = `<div class="empty">ยังไม่มีรายชื่อหุ้นล่าสุด<br><br>ต้องประมวลผลข้อมูลรอบล่าสุดก่อน ระบบจึงจะแสดงรายชื่อหุ้นสำหรับตรวจต่อ</div>`;
    return;
  }
  ...
  host.innerHTML = `
    <div class="tiles" style="margin-bottom:16px">
      <div class="tile hero"><div class="lab">อันดับ 1 ล่าสุด</div><div class="val" style="font-size:26px">${esc(rows[0].symbol)}</div>
        <div class="sub">คะแนนนวัตกรรม ${fmt(rows[0].innovation_alpha_score, 1)} · ${esc(rows[0].industry)}</div></div>
      ...
      <div class="tile"><div class="lab">คะแนนเฉลี่ยของ Top 5</div><div class="val">${fmt(rows.reduce((s, r) => s + r.innovation_alpha_score, 0) / rows.length, 1)}</div>
        <div class="sub">คะแนนแนวโน้มนวัตกรรม</div></div>
      <div class="tile"><div class="lab">สถานะข้อมูล</div><div class="val" style="font-size:22px">${mv.status === "demo_generated" ? "Demo" : "Live"}</div>
        <div class="sub">รายชื่อเพื่อใช้ตรวจต่อ</div></div>
    </div>
    <div class="grid g2">
      <div class="card">
        <h3>หุ้นที่น่าสนใจ 5 อันดับแรก</h3>
        <p class="desc">ใช้เป็น shortlist สำหรับนักวิเคราะห์ตรวจข่าว งบการเงิน สภาพคล่อง และความเหมาะสมก่อนตัดสินใจ</p>
        <table><thead><tr><th>#</th><th>Symbol</th><th>อุตสาหกรรม</th><th class="n">Score</th><th class="n">สัญญาณโมเดล</th><th>ที่มาของคะแนน</th></tr></thead><tbody>
      ...
      <div class="card">
        ...
        <div class="note warn" style="margin-top:12px"><b>ไม่ใช่คำแนะนำซื้อขาย และไม่ทำนายราคาหุ้น</b><br>
          หน้านี้เป็นเครื่องมือคัดกรองหุ้นที่ควรตรวจต่อ ต้องพิจารณาข่าว งบล่าสุด สภาพคล่อง ราคา และข้อจำกัดการลงทุนก่อนใช้งานจริง</div>
      </div>
    </div>`;
  wrapTables(host);
}
...
<section class="pane" id="p-iv">
  <div class="kicker">คัดหุ้นเข้าพอร์ต</div>
  <h2 class="sechead">จัดอันดับหุ้นจากสัญญาณนวัตกรรมของผู้บริหารและบอร์ด</h2>
  <p class="secsub">ระบบประเมินข้อความสื่อสารของ CEO/ผู้บริหารร่วมกับโครงสร้างคณะกรรมการ เพื่อจัดหุ้นเป็นคลาส A-E และคะแนน 0-100 สำหรับช่วยคัดเลือกหุ้นที่น่าสนใจเข้าพอร์ต โดยไม่คำนวณหรือคาดการณ์ราคาหุ้นสุดท้าย</p>

  <div class="flow3">
    <div><b>1 · อ่านสัญญาณบริษัท</b><span>ข้อความ CEO + โครงสร้างบอร์ด → คะแนนและคลาส</span></div>
    <div><b>2 · เทียบผลย้อนหลัง</b><span>พอร์ตจำลองหุ้นคะแนนสูงสุด 10 ตัว เทียบ SET Index</span></div>
    <div><b>3 · ได้รายชื่อเพื่อตรวจต่อ</b><span>Top 5 สำหรับนักวิเคราะห์ตรวจราคา ข่าว งบ และสภาพคล่อง</span></div>
  </div>
  ...
  <div class="card" style="margin-bottom:14px">
    <div class="btnrow">
      ...
      <span class="modelPickLabel">มุมมองคะแนน</span>
  ...
  <div class="split2">
    <div class="card"><h3>1 · ตารางจัดอันดับหุ้น</h3>
      <p class="desc">เรียงตามคะแนนและคลาส · คลิกแถวเพื่อดูว่าอะไรเป็นตัวผลักดันคะแนนของหุ้นนั้น</p>
      <div id="ivBody"></div></div>
    <div class="card"><h3>2 · เหตุผลประกอบคะแนน</h3>
      <p class="desc">ดูว่าปัจจัยใด เช่น บอร์ดอิสระ ขนาดบอร์ด อายุงานบอร์ด หรือโทนภาษาผู้บริหาร มีผลต่อคะแนนมากที่สุด</p>
      <div id="ivSens"></div></div>
  </div>
</section>

<section class="pane" id="p-bt">
  <div class="kicker">ตรวจผลย้อนหลัง</div>
  <h2 class="sechead">พอร์ตจำลองหุ้นคะแนนสูง เทียบ SET Index</h2>
  <p class="secsub">ตรวจว่าหากเลือกหุ้นที่ได้คะแนนสูงสุด 10 ตัวในแต่ละรอบ ผลตอบแทนย้อนหลังจะชนะหรือแพ้ SET Index อย่างไร ใช้เพื่อประเมินคุณค่าของสัญญาณ ไม่ใช่เพื่อทำนายราคาหุ้นรายตัว</p>
  <div class="card"><div class="btnrow modelTopbar" style="margin-bottom:14px"><div style="flex:1"></div><span class="modelPickLabel">มุมมองคะแนน</span><div class="segbar engpick"></div></div>
    <div id="btBody"></div></div>
</section>

<section class="pane" id="p-live">
  <div class="kicker">Shortlist ล่าสุด</div>
  <h2 class="sechead">หุ้นที่ควรนำไปตรวจต่อ 5 อันดับแรก</h2>
  <p class="secsub">รายชื่อจากข้อมูลรอบล่าสุดสำหรับนักวิเคราะห์หรือนักลงทุนใช้ตรวจต่อ ไม่ใช่คำสั่งซื้อขาย และไม่ใช่การคาดการณ์ราคาหุ้น</p>
  <div id="liveBody"></div>
</section>
...
<section class="pane" id="p-dm">
  ...
  <div class="card" style="margin-bottom:14px"><h3>4 · รายละเอียดเทคนิคสำหรับทีมข้อมูล</h3>
    <p class="desc">ส่วนนี้ใช้โดยทีมข้อมูลเพื่อประมวลผลชุดข้อมูลจริง: ข้อความ CEO, ตัวแปรบอร์ด, ผลตอบแทนหุ้น และ SET Index ก่อนนำผลมาแสดงใน dashboard ผู้ใช้</p>
    ...
    <div class="btnrow" style="margin-top:14px">
      <button class="btn s" id="btnTplText">template: ข้อความ CEO</button>
      <button class="btn s" id="btnTplBoard">template: โครงสร้างบอร์ด</button>
      <button class="btn s" id="btnTplReturns">template: ผลตอบแทนหุ้น</button>
...
<section class="pane" id="p-guide">
  <div class="kicker">คู่มือใช้งาน</div>
  <h2 class="sechead">ระบบนี้ช่วยคัดหุ้นเข้าพอร์ตอย่างไร</h2>
  <p class="secsub">ระบบนี้ออกแบบสำหรับนักลงทุนและบริษัทจัดสรรการลงทุน ใช้จัดอันดับหุ้นจากสัญญาณนวัตกรรมของผู้บริหารและบอร์ด เพื่อช่วยเลือกหุ้นไปตรวจต่อ ไม่ได้ทำนายราคาหุ้นสุดท้าย</p>

  <div class="guideHero">
    <div>
      <span class="guideBadge">งานที่ 1</span>
      <h3>เพิ่มบริษัทจากรายงาน</h3>
      <p>ใช้เมื่อต้องการเพิ่มหุ้นใหม่จากรายงานประจำปีหรือข้อความ CEO ระบบจะอ่านเนื้อหาและช่วยเติมค่าตั้งต้นก่อนนำไปจัดอันดับ</p>
      <button class="btn p" id="btnGuideOrg">ไปเพิ่มรายงาน</button>
    </div>
    <div>
      <span class="guideBadge">งานที่ 2</span>
      <h3>คัดหุ้นเข้าพอร์ต</h3>
      <p>ใช้ดูคะแนน 0-100, คลาส A-E, ผลทดสอบย้อนหลังเทียบ SET Index และรายชื่อหุ้นที่ควรนำไปตรวจต่อ</p>
      <button class="btn p" id="btnGuideInv">ไปคัดหุ้นเข้าพอร์ต</button>
    </div>
  </div>

  <div class="guideSteps">
    <div class="guideStep"><b>1</b><h3>เลือกหุ้นที่น่าสนใจ</h3><p>ดูคะแนนและคลาสของหุ้นแต่ละตัวจากข้อความผู้บริหารและโครงสร้างบอร์ด</p></div>
    <div class="guideStep"><b>2</b><h3>ดูเหตุผลประกอบ</h3><p>ตรวจว่าคะแนนมาจากปัจจัยใด เช่น บอร์ดอิสระ ขนาดบอร์ด อายุงานบอร์ด หรือโทนภาษา CEO</p></div>
    <div class="guideStep"><b>3</b><h3>ตรวจต่อก่อนตัดสินใจ</h3><p>ดูผลย้อนหลังเทียบ SET แล้วตรวจข่าว งบ ราคา และสภาพคล่องก่อนนำเข้าพอร์ต</p></div>
  </div>

  <div class="grid g2" style="margin-top:16px">
    <div class="card">
      <h3>อ่านหน้าคัดหุ้นอย่างไร</h3>
      <div class="readList">
        <div><b>คะแนน 0-100</b><span>ยิ่งสูง แปลว่าหุ้นนั้นมีสัญญาณนวัตกรรมเด่นกว่าเมื่อเทียบกับกลุ่มที่ใช้ประเมิน</span></div>
        <div><b>คลาส A-E</b><span>Class A คือกลุ่มบนสุด เหมาะสำหรับนำไปตรวจต่อก่อนหุ้นคลาสต่ำกว่า</span></div>
        <div><b>ผลย้อนหลังเทียบ SET</b><span>ดูว่ากลยุทธ์เลือกหุ้นคะแนนสูงสุดเคยสร้างผลตอบแทนเหนือ SET Index หรือไม่</span></div>
      </div>
    </div>
    <div class="card">
      <h3>คำที่ควรใช้ตอนนำเสนอ</h3>
      <div class="note info"><b>ประโยคสั้น ๆ</b><br>ระบบนี้ช่วยคัดหุ้นที่มีแนวโน้มด้านนวัตกรรมสูงจากข้อความ CEO และโครงสร้างบอร์ด เพื่อให้นักลงทุนเลือกหุ้นไปตรวจต่ออย่างเป็นระบบ</div>
      <div class="note warn" style="margin-top:10px"><b>ประโยคข้อจำกัด</b><br>ระบบจัดอันดับและแนะนำกลุ่มหุ้นที่น่าสนใจเท่านั้น ไม่คำนวณราคาหุ้นสุดท้าย ไม่ใช่คำแนะนำซื้อขาย และไม่ยืนยันเหตุและผล</div>
    </div>
  </div>
</section>

<section class="pane" id="p-howto">
  <div class="kicker">Checklist</div>
  <h2 class="sechead">ต้องทำอะไรบ้างก่อนนำเสนอ</h2>
  <p class="secsub">ใช้หน้านี้เป็นรายการตรวจงาน เพื่อให้ demo ดูครบและตอบคำถามกรรมการได้ชัด</p>

  <div class="checkGrid">
    <div class="card"><h3>ข้อมูลที่ต้องมี</h3>
      <label class="check"><input type="checkbox"> Public text ของ SET100 รายไตรมาส</label>
      <label class="check"><input type="checkbox"> Board variables 4 ตัว: female, independent, size, tenure</label>
      <label class="check"><input type="checkbox"> ผลตอบแทนหุ้นรายไตรมาสย้อนหลัง 3-5 ปี</label>
      <label class="check"><input type="checkbox"> ผลตอบแทน SET Index ช่วงเดียวกัน</label>
    </div>
    <div class="card"><h3>สิ่งที่ระบบต้องแสดง</h3>
      <label class="check"><input type="checkbox"> คะแนน 0-100 และคลาส A-E</label>
      <label class="check"><input type="checkbox"> ผลย้อนหลังของกลุ่มหุ้นคะแนนสูง เทียบ SET Index</label>
      <label class="check"><input type="checkbox"> Equity curve และ alpha รวม</label>
      <label class="check"><input type="checkbox"> รายชื่อหุ้น Top 5 สำหรับตรวจต่อ</label>
    </div>
    <div class="card"><h3>คำเตือนที่ต้องพูด</h3>
      <label class="check"><input type="checkbox"> ไม่ใช่คำแนะนำลงทุน</label>
      <label class="check"><input type="checkbox"> ไม่คำนวณหรือคาดการณ์ราคาหุ้นสุดท้าย</label>
      <label class="check"><input type="checkbox"> ไม่ใช่ความสัมพันธ์เชิงสาเหตุ</label>
      <label class="check"><input type="checkbox"> ต้องตรวจข่าว งบ และสภาพคล่องก่อนใช้จริง</label>
      <label class="check"><input type="checkbox"> Demo data ต้องแยกจาก real pipeline output</label>
    </div>
  </div>

  <div class="card" style="margin-top:16px">
    <h3>ลำดับการ demo ที่ง่ายที่สุด</h3>
    <ol class="demoFlow">
      <li><b>เปิด คู่มือใช้งาน</b> อธิบายว่าระบบช่วยคัดหุ้นเข้าพอร์ต ไม่ได้ทำนายราคาหุ้น</li>
      <li><b>ไป คัดหุ้นเข้าพอร์ต</b> แสดงคะแนน 0-100 และคลาส A-E</li>
      <li><b>ไป ผลย้อนหลังเทียบ SET</b> อธิบายว่ากลุ่มหุ้นคะแนนสูงเคยชนะหรือแพ้ SET Index อย่างไร</li>
      <li><b>ไป หุ้นที่ควรตรวจต่อ 5 อันดับแรก</b> แสดง shortlist สำหรับนักวิเคราะห์</li>
      <li><b>จบที่ ข้อมูลและโมเดล</b> แสดงว่าระบบประมวลผลจากข้อมูลข้อความ CEO โครงสร้างบอร์ด และผลตอบแทนย้อนหลัง</li>
    </ol>
  </div>
</section>`;
...
.guideHero{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:16px}
.guideHero>div{background:#fff;border:1px solid var(--line);border-radius:8px;padding:18px;box-shadow:var(--sh)}
.guideHero h3{margin:6px 0 5px;font-size:20px}
.guideHero p{margin:0 0 14px;color:var(--grey);font-size:13px;line-height:1.6}
.guideBadge{display:inline-block;font-size:11px;color:#0d7a5c;background:var(--mint);border-radius:999px;padding:2px 9px}
.guideSteps{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}
.guideStep{background:var(--ink);color:#fff;border-radius:8px;padding:16px;min-height:132px}
.guideStep b{display:grid;place-items:center;width:28px;height:28px;border-radius:999px;background:var(--accent);margin-bottom:12px}
.guideStep h3{margin:0 0 4px;font-size:15px}
.guideStep p{margin:0;color:#b9c8d4;font-size:12px;line-height:1.55}
.readList{display:grid;gap:10px}
.readList>div{border-bottom:1px solid var(--line);padding-bottom:10px}
.readList>div:last-child{border-bottom:0;padding-bottom:0}
.readList b{display:block;font-size:13px}
.readList span{display:block;color:var(--grey);font-size:12px;line-height:1.5}
.checkGrid{display:grid;grid-template-columns:repeat(3,1fr);gap:14px}
.check{display:flex;gap:9px;align-items:flex-start;border-bottom:1px solid var(--line);padding:9px 0;font-size:12.5px;color:var(--grey)}
...
function bootV2() {
  ...
  const wanted = [
    ...
    ["group", "คัดหุ้นเข้าพอร์ต"],
    ["iv", mk("iv", "คะแนน", "จัดอันดับหุ้น")],
    ["bt", mk("bt", "ย้อนหลัง", "เทียบ SET Index")],
    ["live", mk("live", "Shortlist", "หุ้นที่ควรตรวจต่อ")],
    ["group", "ระบบ"],
    ["guide", mk("guide", "เริ่มต้น", "คู่มือใช้งาน")],
    ["howto", mk("howto", "นำเสนอ", "Checklist ก่อนส่ง")],
  ...
  const guideOrg = $("#btnGuideOrg");
  if (guideOrg) guideOrg.onclick = () => { setMode("org", true); go("up"); };
  const guideInv = $("#btnGuideInv");
  if (guideInv) guideInv.onclick = () => { setMode("inv", true); go("iv"); };
